...
fn main() {
    ...
    new_path.append(&mut vec!(start_space));
    for y in 0..grid.len() {
        for x in 0..grid[y].len() {
            if !new_path.contains(&[y, x]) {
                grid[y][x] = GridSpace::Ground;
            }
            print!("{}", match grid[y][x] {
                GridSpace::Pipe { pipe, distance } => {
                    match pipe {
                        Pipe::Vertical => '|',
                        Pipe::Horizontal => '-',
    ...
    grid[start_space[0]][start_space[1]] = GridSpace::Pipe { pipe: Pipe::Horizontal, distance: 0 };

    let mut area = 0;
    for y in 0..grid.len() {
        let mut is_inside = false;
        let mut horizontal_beginning: Option<Pipe> = None;
        for x in 0..grid[y].len() {
            if new_path.contains(&[y, x]) {
                match grid[y][x] {
                    GridSpace::Pipe { pipe, distance } => {
                        match pipe {
                            ...
                            Pipe::NorthEast => {
                                horizontal_beginning = Some(Pipe::NorthEast);
                            }
                            Pipe::SouthEast => {
                                horizontal_beginning = Some(Pipe::SouthEast);
                            }
                            Pipe::NorthWest => {
                                if let Some(i) = horizontal_beginning {
                                    match i {
                                        Pipe::SouthEast => {
                                            is_inside = !is_inside;
                                        }
